...
class MECEnv:
    ...
    def reset(self):
        ...
        self.t = 0
        # 随机生成任务参数
        self.d = np.random.uniform(10, 30, (self.N_BS, self.M_UE, T))  # 任务大小 [10,30] Mbit
...
for episode in range(EPISODES):
    ...
    for t in range(T):
        ...
        for j in range(N_BS):
            ...
            a = actors[j](o).detach().cpu().numpy()  # 取回cpu做后续numpy操作
            # 动作后K位归一化到功率范围
    ...
    print(f"Episode {episode + 1}, 平均奖励: {episode_rewards.mean():.2f}, 命中率: {hit_rate:.3f}, 能耗: {episode_energy:.2f}, 时延: {episode_delay:.2f}")
# ------------------ 结果可视化 ------------------
