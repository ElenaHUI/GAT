...
class MECEnv:
    def __init__(self):
        ...
        self.hit_count = 0
        self.total_count = 0
        self.lfu_cache = [{} for _ in range(self.N_BS)]  # LFU缓存结构，key为内容，value为频率
    ...
    def step(self, actions):
        ...
        for j in range(self.N_BS):
            ...
            for i in range(self.M_UE):
                ...
                q_ = next_q[j][i]
                if k in self.lfu_cache[j]:
                    # 缓存命中，按原逻辑
                    self.hit_count += 1
                    self.lfu_cache[j][k] += 1  # 命中时频率+1
    ...
    def update_lfu(self, j, k):
        # j:基站编号, k:服务类型
        used = sum([self.c_k[kk] for kk in self.lfu_cache[j].keys()])
        # 如果已在缓存中，频率+1
        if k in self.lfu_cache[j]:
            self.lfu_cache[j][k] += 1
        else:
            # 不在缓存中，需判断空间
            while self.lfu_cache[j] and used + self.c_k[k] > self.C_j[j]:
                # 找到频率最小的内容，若有多个，移除最早插入的
                min_freq = min(self.lfu_cache[j].values())
                min_keys = [kk for kk, v in self.lfu_cache[j].items() if v == min_freq]
                # 这里简单移除第一个最小频率的内容
                del_key = min_keys[0]
                used -= self.c_k[del_key]
                del self.lfu_cache[j][del_key]
            self.lfu_cache[j][k] = 1  # 新内容频率为1
        # 更新beta
        self.beta[j] = np.zeros(self.K, dtype=int)
        for kk in self.lfu_cache[j].keys():
            self.beta[j, kk] = 1
